...
def get_advent_of_code_input(year, day, session_cookie):
    ...
    if not os.path.exists(os.path.join(BASE_DIR, f'{year}', f'{day:02d}', 'task.txt')):
        ...
        print(f"{os.path.join(BASE_DIR, f'{year}', f'{day:02d}', 'task.txt')} already exists, skipping download.")
    
    puzzle_input = None
    if not os.path.exists(os.path.join(BASE_DIR, f'{year}', f'{day:02d}', 'input.txt')):
...
def write_files(year, day, task_description, puzzle_input):
    # Ensure the directories exist
    os.makedirs(os.path.join(BASE_DIR, f'{year}', f'{day:02d}'), exist_ok=True)
    
    # Write the task description and puzzle input to files
    if task_description is not None:
        with open(os.path.join(BASE_DIR, f'{year}', f'{day:02d}', 'task.txt'), 'w') as task_file:
            task_file.write(task_description)
        print(f"Task description written to {os.path.join(BASE_DIR, f'{year}', f'{day:02d}', 'task.txt')}")
    
    if puzzle_input is not None:
        with open(os.path.join(BASE_DIR, f'{year}', f'{day:02d}', 'input.txt'), 'w') as data_file:
            data_file.write(puzzle_input)
        print(f"Puzzle input written to {os.path.join(BASE_DIR, f'{year}', f'{day:02d}', 'input.txt')}")
